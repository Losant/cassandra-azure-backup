...
#
#
# Description :  Take snapshot and incremental backups of Cassandra and copy them to Azure Storage Account
...
function print_usage() {
  echo "Cassandra Backup to Azure Storage Account Version: ${VERSION}"
  cat <<'EOF'
Usage: ./cassandra-cloud-backup.sh [ options ] <command>
Description:
  Utility for creating and managing Cassandra Backups with Azure Storage Account.
...
Flags:
  ...
  -D, --download-only
    During a restore this will only download the target files from Azure
  ...
  -r,  restore
    Restore a backup, requires a --azbucket path and optional --backupdir
...
Examples:
  ...
  upload into the Azure Storage Account, and clear old incremental and snapshot files
  ...
  List inventory of available backups stored in Azure Store Account
...
function validate() {
  ...
  if [ ${ACTION} != "inventory" ]; then
    ...
    if [ ${ACTION} = "restore" ]; then
      AZ_LS=$(${AZCLI} s3 ls ${AZ_BUCKET} | head -n1)
      loginfo "Azure first file listed: ${AZ_LS}"
      if  grep -q 'incr' <<< "${AZ_LS}"; then
        ...
        if grep -q 'snpsht' <<< "${AZ_LS}"; then
          loginfo "Detected full snapshot backup requested for restore."
        else
          logerror "Detected a Azure Storage Account path that is not a backup" \
          " location. Make sure the --azbucket e is the full path to a specific backup"
        fi
      fi
      if grep -q "tgz" <<< "${AZ_LS}"; then
        loginfo "Detected compressed .tgz file for restore"
        COMPRESSION=true
        TAR_EXT="tgz"
        TAR_CFLAG="-z"
      fi
      if  grep -q "tbz" <<< "${AZ_LS}"; then
        loginfo "Detected compressed .tbz file for restore"
        COMPRESSION=true
        TAR_EXT="tbz"
        TAR_CFLAG="-j"
      fi
      if  grep -q "tar" <<< "${AZ_LS}"; then
        loginfo "Detected uncompressed .tar file for restore"
        COMPRESSION=false
        TAR_EXT="tar"
        TAR_CFLAG=""
      fi     
      RESTORE_FILE=$(awk -F"/" '{print $NF}' <<< "${AZ_LS}")
      if [[ "${RESTORE_FILE}" != *.${TAR_EXT} ]] ; then
          ...
          if [[ "${RESTORE_FILE}" ==  ${TAR_EXT}-* ]]; then
            ...
            logerror "Restore is not a tar file  ${AZ_BUCKET}"
          fi
      fi
      if [[ ! ${AZ_BUCKET} =~ ^.*\.${TAR_EXT}$ ]]; then
        if ${SPLIT_FILE}; then
          #remove the trailing digits and replace the suffix
          RESTORE_FILE="${RESTORE_FILE%${SUFFIX}*}${SUFFIX}*"
          AZ_BUCKET="${AZ_BUCKET%/}/${RESTORE_FILE}"
        else
          AZ_BUCKET="${AZ_BUCKET%/}/${RESTORE_FILE}"
          loginfo "Fixed up restore bucket path: ${AZ_BUCKET}"
        fi
      fi
...
function verbose_vars() {
  ...
  logverbose "AZ_TMPDIR: ${AZ_TMPDIR}"
...
# to create the backup set and then push it to Azure
function backup() {
  create_azure_backup_path
...
function create_azure_backup_path() {
  AZ_BACKUP_PATH="${AZ_BUCKET}/backups/${HOSTNAME}/${SUFFIX}/${DATE}/"
...
#which allows for more efficient upload / download from Azure Storage Account
...
# Copy the backup files up to the Azure Storage Account
function copy_to_azure() {
  loginfo "Copying files to ${AZ_BACKUP_PATH}"
  if ${DRY_RUN}; then
    if ${SPLIT_FILE}; thenz
      loginfo "DRY RUN: ${AZCLI} s3 cp ${COMPRESS_DIR}/${SPLIT_FILE_SUFFIX}* ${AZ_BACKUP_PATH}"  # CHANGE THIS
    else
      loginfo "DRY RUN: ${AZCLI} s3 cp ${COMPRESS_DIR}/${ARCHIVE_FILE} ${AZ_BACKUP_PATH}"
    fi
  else
    if ${SPLIT_FILE}; then
...
function restore_get_files() {
  ...
  if ${SPLIT_FILE}; then
    restore_split_from_azure
  else
    restore_compressed_from_azure
  fi
...
function restore_split_from_azure() {
  ...
  if ${DRY_RUN}; then
    ...
    ${AZCLI} --recursive s3 cp  "${AZ_BUCKET}" "${COMPRESS_DIR}" # CHANGE THIS
  fi
  restore_split
}
...
HOSTNAME=${HOSTNAME:-"$(hostname)"} #used for azure backup location
